...
%  Input
%    tabvals   : n.1 sorted vector
%    x         : array of values
%    endadj    : optional endpoint adjustment: 0, 1, 2 or 3.
%  Output
%    ind     : array (same size as x) of indices from 1 to n
%    Returns array of size(x) with element (i,j) equal to 
%    max k: x(i,j)>=tabvals(k)
%  Optional endpoint adjustments:
%    0: no adjustments
%    1: values of x < min(tabvals) will return length(tabvals=tabvals(1))
%    2: values of x > max(tabvals) will return m-length(tabvals=tabvals(end))
...
function ind = lookupfor(tabvals,x,endadj)
