...
% This file solves uncertainty case with Quadratic
% Weimin Zhou
%% Quadratic, finite, uncertainty

T=45;
gamma=0.96;
prob=[(1+gamma)/2 (1-gamma)/2;
    (1-gamma)/2 (1+gamma)/2]; %transition matrix
...
cbar = 100;
util=@(x) -.5*(x-cbar).^2;         %quadratic utility function
...
for i=1:T  %corresponding to period 45-i
    ...
    tempv=[];
    tempind=[];
    for a=[(1-sigma_y)*(R^(-1-i)-1)/r:0.01:3] %state variable: initial wealth "a"
        j=j+1;
        b=[(1-sigma_y)*(R^(-i)-1)/r:0.01:3]';  % possible choice(policy function a'(a)) for T-i+1
...
title('cunsumption policy - Quadratic')
legend('1-\sigma_y','1+\sigma_y')
xlabel('a') 
ylabel('c_5 (a,y)') 
saveas(gcf,'41qa.png')
%% compare sigma_y from .1 to .5
%
figure
subplot(1,2,1)
plot(a_1, ch_1, a_5, ch_5);
title('cunsumption policy - Quadratic (good shock)')
legend('\sigma_y=0.1','\sigma_y=0.5')
xlabel('a') 
ylabel('c(a,y)') 
subplot(1,2,2)
plot(a_1, cl_1, a_5, cl_5);
title('cunsumption policy - Quadratic (bad shock)')
legend('\sigma_y=0.1','\sigma_y=0.5')
xlabel('a') 
ylabel('c(a,y)') 
saveas(gcf,'423-q.png')
%}
...
for t=0:T-1
    c(1,t+1)=t;
    c(2,t+1)=R*a+Y_realize(t+1)-((Index{2*t+Y_state(t+1)}(int16((a-(1-sigma_y)*(R^(t-T-1)-1)/r)/0.01+1))-1)*0.01+(1-sigma_y)*(R^(t-T)-1)/r);
...
figure
plot(c(1,:),c(2,:))
title('Cunsumption profile for 45 periods-quadratic utility')
xlabel('periods t') 
ylabel('c(t)') 